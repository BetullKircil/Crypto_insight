package com.betulkircil.cryptoinsight.utils


//https://api.coingecko.com/api/v3/coins/markets?vs_currency=eur  coins api
//https://newsapi.org/v2/everything?q=cryptoCurrency&sortBy=publishedAt&apiKey=0685e2c571b9424cb002c1a1ced2ee42
//https://api.coingecko.com/api/v3/nfts/list
//https://api.coingecko.com/api/v3/exchanges

object Constants {
    const val COIN_BASE_URL = "https://api.coingecko.com/api/v3/"
    const val COIN_END_POINT = "coins/markets"
    const val API_KEY = "0685e2c571b9424cb002c1a1ced2ee42"
    const val NEWS_BASE_URL = "https://newsapi.org/"
    const val NEWS_END_POINT = "v2/everything"
    const val ServerClient = "623821793257-ug0mjriq9d39vraqb0lqdkel03fjkfao.apps.googleusercontent.com"
    const val COINS_CATEGORY_URL = "https://economictimes.indiatimes.com/markets/cryptocurrency"
    const val BLOCKCHAIN_CATEGORY_URL = "https://economictimes.indiatimes.com/topic/blockchain-technology"
    const val NFT_CATEGORY_URL = "https://timesofindia.indiatimes.com/topic/nft/news"
    const val WEB3_CATEGORY_URL = "https://economictimes.indiatimes.com/topic/web3"
    const val METAVERSE_CATEGORY_URL = "https://economictimes.indiatimes.com/topic/metaverse-tech"
    const val DAO_CATEGORY_URL = "https://economictimes.indiatimes.com/topic/decentralized-autonomous-organization-dao"
    const val FINANCE_CATEGORY_URL = "https://timesofindia.indiatimes.com/business"
}





